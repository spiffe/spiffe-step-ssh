...
CA_PID=""
run_setup() {
  echo "Running/Rerunning configuration steps..."
  
  cd "/var/run/spiffe/step-ssh-server/$INSTANCE"
  mkdir -p "/var/run/spiffe/step-ssh-fetchca/$INSTANCE/fetchca"
  chmod 755 "/var/run/spiffe/step-ssh-fetchca/$INSTANCE/fetchca"
  ...
  if [ -n "$CA_PID" ] && kill -0 "$CA_PID" 2>/dev/null; then
    echo "Sending SIGHUP to step-ca (PID: $CA_PID)"
    kill -SIGHUP "$CA_PID"
  fi
}
trap 'run_setup' SIGHUP
run_setup
